...
def run(plan, nodes, suite = "all"):
    ...
    if suite == "all" or suite == "sync":
        ...
        results["sync"] = sync.run(plan, nodes)

    if suite == "all" or suite == "consensus":
        plan.print("=== Running consensus tests ===")
        results["consensus"] = consensus.run(plan, nodes)
